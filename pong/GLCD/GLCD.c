...
extern unsigned short pallina_Y,pallina_X,AD_current,AD_last, increment_x, increment_y, AD_curr2;
...
int muro(void){
	if((pallina_X + 2 >= 234 ) &&(pallina_Y>=42 && pallina_Y<=277)){			//parete dx
		return 1;
	}
	else if (( pallina_X -2 <= 5) &&(pallina_Y>=42 && pallina_Y<=277)){		//parete sx
		return 1;
	}
	else if((pallina_Y>=START_PADDLE_Y0 && pallina_Y<START_PADDLE_Y0+9) && (pallina_X +2>=AD_current-25 && pallina_X -2< AD_current+25 ))//sono dentro il paddle1, game over giocatore 1
		return 5;
	else if(((pallina_Y>278 && (pallina_X-2<=4 || pallina_X+2>=235)) || pallina_Y>288) && (AD_current + 25< pallina_X-2 || AD_current -25 > pallina_X +2))//game over giocatore 1
		return 5;
	else if(((pallina_Y+4<32 && (pallina_X-2<=4 || pallina_X+2>=235)) || pallina_Y<15) || (pallina_Y+4< 41 && (pallina_X+2>=AD_current-25 && pallina_X -2<= AD_current+25  ) ) )//game over giocatore 2
		return 6;
	return 0;
}

int paddle(void){
	if(( pallina_Y +4==START_PADDLE_Y0-1) && (pallina_X +2>=AD_current-25 && pallina_X -2<= AD_current+25 ))
		return 1;
	if((pallina_Y+4>=START_PADDLE_Y0 && pallina_Y<=288 ) && (pallina_X +2==AD_current-25 || pallina_X -2== AD_current+24 ))
		return 3;
	else if((pallina_Y+4>=START_PADDLE_Y0 && pallina_Y+1<START_PADDLE_Y0) && (pallina_X +2>=AD_current-25 && pallina_X -2< AD_current+25 ))//sono dentro il paddle1, ma non del tutto, solo in parte
		return 2;
	else if ((pallina_Y == 42 ) && (pallina_X +2>=AD_curr2 -25 && pallina_X -2<= AD_curr2 +25 ))	//colpisce paddle 2
		return 4;
	else if((pallina_Y<=42 && pallina_Y+4>=32 ) && (pallina_X +2==AD_curr2-25 || pallina_X -2== AD_curr2+24 ))	//colpisce paddle 2 lateralmente
		return 5;
	else if((pallina_Y<=41 && pallina_Y+4>=41) && (pallina_X +2>=AD_curr2-25 && pallina_X -2< AD_curr2+25 ))//sono dentro il paddle2, ma non del tutto, solo in parte
		return 6;
	return 0;
}

void incremento_coord(int *x, int *y,int angle){
	if(angle>=20 && angle<= 23){
			*x=3;
			*y=-1;
		}
		else if( angle>23 && angle <=33){
			*x=2;
			*y=-1;
		}
		else if( angle>33 && angle <=53){
			*x=1;
			*y=-1;
		}
		else if( angle>53 && angle<= 60){
			*x=2;
			*y=-3;
		}
		else if ( angle>60 && angle <=68 ){
			*x=1;
			*y=-2;
		}
		else if ( angle>68 && angle<= 74){
			*x=1;
			*y=-3;
		}
		else if (angle > 74 && angle <= 77){
			*x=1;
			*y=-4;
		}
		else if( angle >77 && angle <=79){
			*x=1;
			*y= -5;
		}
		else if(angle>=101 && angle<103){
			*x=-1;
			*y= -5;
		}
		else if(angle>=103 && angle<106){
			*x=-1;
			*y=-4;
		}
		else if(angle>=106 && angle <112){
			*x=-1;
			*y=-3;
		}
		else if(angle >=112 && angle < 120){
			*x=-1;
			*y=-2;
		}
		else if(angle>=120 && angle <127){
			*x=-2;
			*y=-3;
		}
		else if( angle>=127 && angle < 147){
			*x=-1;
			*y=-1;
		}
		else if( angle >=147 && angle < 157){
			*x=-2;
			*y=-1;
		}
		else if(angle>=157 && angle <=160){
			*x=-3;
			*y=-1;
		}
}

void change_ball(int *x, int *y){
	if(*x!=0 && *y!=0){
...
void bordo_rosso(void){
	int i;
	for(i=0; i<5; i++){
		LCD_DrawLine(i, 0, i, 319,Red);				//sx
		LCD_DrawLine(i+235, 0, i+235, 319,Red);	//dx
	}
}
